...
INT32 ffsMountVol(struct super_block *sb, INT32 drv)
{
	...
	if (i < 53) {
		brelse(tmp_bh);
		bdev_close(sb);
		printk(KERN_ERR "[EXFAT] FAT16/FAT32 is not compatilbe in this module.\n");
		return  FFS_FORMATERR;
						   ...
						   UINT32 start_clu, UINT64 size)
{
	...
	strm_ep = (STRM_DENTRY_T *) get_entry_in_dir(sb, p_dir, entry+1, &sector1);
	if (!strm_ep)
		return FFS_MEDIAERR;
...
INT32 resolve_path(struct inode *inode, UINT8 *path, CHAIN_T *p_dir, UNI_NAME_T *p_uniname)
{
	...
	STRCPY(p_fs->name_buf, path);

	nls_cstring_to_uniname(sb, p_uniname, p_fs->name_buf, &lossy);
	if (lossy)
		return(FFS_INVALIDPATH);
