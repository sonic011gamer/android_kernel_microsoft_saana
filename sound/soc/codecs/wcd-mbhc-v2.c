...
static DEVICE_ATTR(btn_state, 0664, show_btn_state, NULL);
#endif
...
/*@20150325, add for support iphone headset*/ //FAO-28
				...
				const enum wcd_mbhc_cs_mb_en_flag cs_mb_en)
{
	...
	case WCD_MBHC_EN_MB:
		...
		/*
		20150603,  set bit 6 and 7 as 1 for pull up mode
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_MICB_CTRL, 2);
		*/
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_MICB_CTRL, 3);
...
static int wcd_mbhc_get_button_mask(struct wcd_mbhc *mbhc)
{
	...
	case 0:
		mask = SND_JACK_BTN_0;
		/*@20150325, add debug for headset buttons */ //FAO-28
		pr_info("%s:pressed buttons  0\n", __func__); 
		break;
	case 1:
		mask = SND_JACK_BTN_1;
		pr_info("%s:pressed buttons  1\n", __func__); 
		break;
	case 2:
		mask = SND_JACK_BTN_2;
		pr_info("%s:pressed buttons  2\n", __func__); 
		break;
	case 3:
		mask = SND_JACK_BTN_3;
		pr_info("%s:pressed buttons  3\n", __func__); 
		break;
	case 4:
		mask = SND_JACK_BTN_4;
		pr_info("%s:pressed buttons  4\n", __func__); 
		break;
	case 5:
		mask = SND_JACK_BTN_5;
		break;
...
//2015.3.12, Add switch device for FTM request FAO-4
#ifdef LEGACY_SWITCH_DEV_SUPPORT
	/*@20150312, Modify this function for MCS-4331 kernel_panic issue*/
