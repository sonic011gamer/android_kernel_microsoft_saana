...
extern int SVI_enable;		//E1M- Add SVI(AIE) setting
...
/* E1M-576 - gatycclu - Add LCM mipi reg read/write command */
extern void fih_get_read_reg (char *reg_val);
extern void fih_set_read_reg (unsigned int reg, unsigned int reg_len);
extern void fih_set_write_reg (unsigned int len, char *data);
extern void fih_fb_suspend (int enable);
extern int fih_set_esd (bool enable);
extern bool fih_get_esd (void);
/* end E1M-576 */
/* E1M-4489 - gatycclu - Add SVI(AIE) setting */
extern int fih_get_svi (void);
extern int fih_set_svi (int svi);
/* end E1M-4489 */

/* E1M-4489 - gatycclu - Add SVI(AIE) setting */
static int fih_lcm_read_svi_settings(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", fih_get_svi());
...
static int fih_lcm_write_svi_settings(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	...
	res = fih_set_svi(simple_strtoull(tmp, NULL, 0));
	if (res < 0){
		return res;
	}
...
static int fih_lcm_svi_settings_open(struct inode *inode, struct file *file)
{
	return single_open(file, fih_lcm_read_svi_settings, NULL);
};

static struct file_operations svi_file_ops = {
	.owner   = THIS_MODULE,
	.open    = fih_lcm_svi_settings_open,
	.read    = seq_read,
	.write	 = fih_lcm_write_svi_settings,
	.llseek  = seq_lseek,
	.release = single_release
};
/* end E1M-4489 */

static int fih_lcm_get_esd_mode(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", fih_get_esd());
...
static int fih_lcm_set_esd_mode(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
	...
	if(simple_strtoull(tmp, NULL, 0) > 0)
	{
		res = fih_set_esd(true);
		if (res < 0)
		{
			return res;
		}
	} else {
		res = fih_set_esd(false);
		if (res < 0)
		{
			return res;
		}
...
static int fih_lcm_esd_mode_open(struct inode *inode, struct file *file)
{
	return single_open(file, fih_lcm_get_esd_mode, NULL);
};

static struct file_operations esd_file_ops = {
	.owner   = THIS_MODULE,
	.open    = fih_lcm_esd_mode_open,
	.read    = seq_read,
	.write	 = fih_lcm_set_esd_mode,
	.llseek  = seq_lseek,
	.release = single_release
};

static int fih_lcm_get_suspend_mode(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", 1);
...
static int fih_lcm_enter_suspend_mode(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
	...
	fih_fb_suspend(simple_strtoull(tmp, NULL, 0));

	return size;
}

static int fih_lcm_suspend_mode_open(struct inode *inode, struct file *file)
{
	return single_open(file, fih_lcm_get_suspend_mode, NULL);
};

static struct file_operations suspend_file_ops = {
	.owner   = THIS_MODULE,
	.open    = fih_lcm_suspend_mode_open,
	.read    = seq_read,
	.write	 = fih_lcm_enter_suspend_mode,
	.llseek  = seq_lseek,
	.release = single_release
};
...
static int fih_lcm_write_color_mode(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
...
static int fih_lcm_write_color_temperature(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[5];
	unsigned int size;
...
static int fih_lcm_write_cabc_settings(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
...
/* E1M-390 - gatycclu - Add error count and status for FIH Run-In */
...
static int fih_lcm_write_vddio(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
...
static int fih_lcm_write_avdd(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
...
static int fih_lcm_write_avee(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
...
static int fih_lcm_write_reset(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
...
static int fih_lcm_write_init(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
...
static int fih_lcm_write_ldos(struct file *file, const char __user *buffer,
	size_t count, loff_t *ppos)
{
	unsigned char tmp[2];
	unsigned int size;
...
/* E1M-4489 - gatycclu - Add SVI(AIE) setting */
	LCM0_svi[] = {
	{"AllHWList/LCM0/SVI", &svi_file_ops},
	{NULL}, },
/* end E1M-4489 */
/* E1M-576 - gatycclu - Add LCM mipi reg read/write command */
	LCM0_reg_read[] = {
...
/* end E1M-390 */
	...
	{NULL}, },
	LCM0_suspend[] = {
	{"AllHWList/LCM0/suspend", &suspend_file_ops},
	{NULL}, },
	LCM0_esd[] = {
	{"AllHWList/LCM0/esd", &esd_file_ops},
	{NULL},
	};
	//SW4-HL-Display-PowerPinControlPinAndInitCodeAPI-00+}_20150519
...
/* E1M-4489 - gatycclu - Add SVI(AIE) setting */
		if (SVI_enable)
		{
			ent = proc_create((LCM0_svi->name) + 15, 0, lcm0_dir, LCM0_svi->ops);
			if (ent == NULL)
			{
				pr_err("\n\nUnable to create /proc/%s", LCM0_svi->name);
			}
			pr_debug("\n\n*** [HL] %s, succeed to create proc/%s ***\n\n", __func__, LCM0_svi->name);
		}
/* end E1M-4489 */
	}
...
/* end E1M-390 */
	...
	//SW4-HL-Display-PowerPinControlPinAndInitCodeAPI-00+}_20150519
	ent = proc_create((LCM0_suspend->name) + 15, 0, lcm0_dir, LCM0_suspend->ops);
	if (ent == NULL)
	{
		pr_err("\n\nUnable to create /proc/%s", LCM0_suspend->name);
	}
	pr_debug("\n\n*** [HL] %s, succeed to create proc/%s ***\n\n", __func__, LCM0_suspend->name);

	ent = proc_create((LCM0_esd->name) + 15, 0, lcm0_dir, LCM0_esd->ops);
	if (ent == NULL)
	{
		pr_err("\n\nUnable to create /proc/%s", LCM0_esd->name);
	}
	pr_debug("\n\n*** [HL] %s, succeed to create proc/%s ***\n\n", __func__, LCM0_esd->name);
...
/* E1M-4489 - gatycclu - Add SVI(AIE) setting */
	if (SVI_enable)
	{
		remove_proc_entry(LCM0_svi->name, NULL);
	}
/* end E1M-4489 */
...
/* end E1M-390 */
	...
	//SW4-HL-Display-AddCTCPanelHX8394FInsideSupport-01+}_20150522
	remove_proc_entry(LCM0_suspend->name, NULL);
	remove_proc_entry(LCM0_esd->name, NULL);
}
