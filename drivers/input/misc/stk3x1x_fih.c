...
/*@ add for ALS Calibration*/
#include "../../../arch/arm/mach-msm/fih/fih_ramtable.h" 
#include <linux/io.h>
static unsigned int proc_addr = FIH_SENSORDATA_BASE;
static unsigned int proc_size = FIH_SENSORDATA_SIZE;
static unsigned int proc_len = 0;
int als_err_count = 0;
...
#define MIN_ALS_POLL_DELAY_NS        60000000
    ...
    #define STK_LT_N_CT              93
    #define STK_HT_N_CT              157
#endif    /* #ifdef STK_TUNE0 */
#define FIH_PSI_CAL_BASE     690
...
struct stk3x1x_data {
    ...
    /*PIN CTRL*/
    struct pinctrl *proximity_pinctrl;
    struct pinctrl_state *proximity_gpio_state_active;
    struct pinctrl_state *proximity_gpio_state_suspend;
/*@ add for ALS Calibration*/
    int32_t als_uncal_lux;
    int32_t als_ratio;
};
...
int proximity_pinctrl_select(bool on, struct stk3x1x_data *sensor);

extern int fih_als_proc_init(void);

/*@ add for ALS Calibration*/
static int read_als_ratio(struct stk3x1x_data *ps_data){
    int32_t *buf;
    unsigned int i;

    buf = (int32_t *)ioremap(proc_addr, proc_size);
    if (buf == NULL) {
        pr_err("%s: ioremap fail\n", __func__);
        pr_info("BBox::UEC; 8::34\n");
        ps_data->als_ratio=0;
        return 0;
    }
    ...
    proc_len = i;
    pr_info("%s: ioremap buf %d len %d\n", __func__,buf[0],proc_len);
    ps_data->als_ratio=buf[0];
    pr_info("[stk3x1x]::ps_data->als_ratio %d\n",ps_data->als_ratio);
    iounmap(buf);
    return 0;
}
...
/*Evenly*/
    ps_data->als_uncal_lux=alscode;
    alscode=(alscode*100)/ps_data->als_ratio;
    return alscode;
}
...
static int32_t stk3x1x_check_pid(struct stk3x1x_data *ps_data)
{
    ...
    if(stk3x1x_read_otp25(ps_data) == 1)
    {
        ...
        //ps_data->p_wv_r_bd_ratio = 1024*3;
    }
...
static int32_t stk3x1x_get_als_reading(struct stk3x1x_data *ps_data)
{
    ...
    ret = stk3x1x_get_state(ps_data);
    if(ret < 0 || !(ret & STK_STATE_EN_ALS_MASK))
        return ps_data->als_code_last;
    ...
    {
        if(als_data < 30 && ps_data->als_code_last > 10000)
        {
...
static ssize_t stk_als_code_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    non_cal = ps_data->als_uncal_lux;
    return scnprintf(buf, PAGE_SIZE, "%d\n", non_cal);
}

static ssize_t stk_als_code_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
    int32_t cal_lux;

    cal_lux = simple_strtoul(buf,NULL,10);
    printk(KERN_ERR "stk_als_code_store :: cal_lux: %d\n", cal_lux);
    if(cal_lux != 0)
        ps_data->als_ratio= cal_lux;
    return size;
}

#ifdef QUALCOMM_PLATFORM
static ssize_t stk_als_enable_set(struct sensors_classdev *sensors_cdev,unsigned int enabled)
{
...
static int stk_ps_poll_delay_set(struct sensors_classdev *sensors_cdev, unsigned int delay_msec)
{
    //struct stk3x1x_data *als_data = container_of(sensors_cdev,struct stk3x1x_data, als_cdev);
    uint64_t value = 0;
...
static ssize_t stk_als_firlen_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    printk(KERN_INFO "%s: sum = %5d \n", __func__, ps_data->fir.sum);
...
#ifdef QUALCOMM_PLATFORM
static ssize_t stk_ps_enable_set(struct sensors_classdev *sensors_cdev,unsigned int enabled)
{
    struct stk3x1x_data *ps_data = container_of(sensors_cdev,struct stk3x1x_data, ps_cdev);
    int err;
...
static ssize_t stk_all_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    return len;
/*
    return scnprintf(buf, PAGE_SIZE, "[0]%2X [1]%2X [2]%2X [3]%2X [4]%2X [5]%2X [6/7 HTHD]%2X,%2X [8/9 LTHD]%2X, %2X [A]%2X [B]%2X [C]%2X [D]%2X [E/F Aoff]%2X,%2X,[10]%2X [11/12 PS]%2X,%2X [13]%2X [14]%2X [15/16 Foff]%2X,%2X [17]%2X [18]%2X [3E]%2X [3F]%2X\n", ps_reg[0], ps_reg[1], ps_reg[2], ps_reg[3], ps_reg[4], ps_reg[5], ps_reg[6], ps_reg[7], ps_reg[8],ps_reg[9], ps_reg[10], ps_reg[11], ps_reg[12], ps_reg[13], ps_reg[14], ps_reg[15], ps_reg[16], ps_reg[17],ps_reg[18], ps_reg[19], ps_reg[20], ps_reg[21], ps_reg[22], ps_reg[23], ps_reg[24], ps_reg[25], ps_reg[26]);
...
static ssize_t stk_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    return scnprintf(buf, PAGE_SIZE, "[PS=%2X] [ALS=%2X] [WAIT=0x%4Xms] [EN_ASO=%2X] [EN_AK=%2X] [NEAR/FAR=%2X] [FLAG_OUI=%2X] [FLAG_PSINT=%2X] [FLAG_ALSINT=%2X]\n",ps_reg[0]&0x01,(ps_reg[0]&0x02)>>1,((ps_reg[0]&0x04)>>2)*ps_reg[5]*6,(ps_reg[0]&0x20)>>5,(ps_reg[0]&0x40)>>6,ps_reg[16]&0x01,(ps_reg[16]&0x04)>>2,(ps_reg[16]&0x10)>>4,(ps_reg[16]&0x20)>>5);
}
...
static ssize_t stk_ps_cali_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    printk("%s: psi_set=%d, psa=%d,psi=%d, word_data=%d\n", __func__,ps_data->psi_set, ps_data->psa, ps_data->psi, word_data);
#ifdef CALI_PS_EVERY_TIME
...
static struct device_attribute als_code_attribute = __ATTR(code, 0664, stk_als_code_show, stk_als_code_store);
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_data *ps_data)
{
    ...
    //pr_info("%s: Enter en=%d,psi_set=%d\n", __func__, ps_data->ps_enabled,ps_data->psi_set);
#ifdef CALI_PS_EVERY_TIME
    if(!(ps_data->ps_enabled))
#else
    ...
    if((ps_data->psi < FIH_PSI_CAL_BASE)&&(diff > ps_data->stk_max_min_diff))
    {
        ...
        if(ps_data->ps_thd_h > ps_data->ps_high_thd_boot)
        {
            ...
            pr_info("%s: update boot HT=%d, LT=%d\n", __func__, ps_data->ps_high_thd_boot, ps_data->ps_low_thd_boot);
        }
#endif
        stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
        stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
#ifdef STK_DEBUG_PRINTF
        pr_info("%s: FAE tune0 psa-psi(%d) > STK_DIFF found\n", __func__, diff);
#endif
        hrtimer_cancel(&ps_data->ps_tune0_timer);
    }
...
//        printk(KERN_INFO "%s: ges_enabled=1, als input event %d lux\n",__func__, ps_data->als_lux_last);
    ...
    if(ps_data->ir_code)
    {
        ...
        if(reading < STK_IRC_MAX_ALS_CODE && reading > STK_IRC_MIN_ALS_CODE && ps_data->ir_code > STK_IRC_MIN_IR_CODE)
        {
...
//  uint8_t disable_flag = 0;
    ...
    if(ps_data->ges_enabled == 2)
    {
        ...
        disable_flag2 = org_flag2_reg & (STK_FLG2_INT_GS_MASK | STK_FLG2_GS10_MASK | STK_FLG2_GS01_MASK);
        ...
        atomic_set(&ps_data->gesture2, (disable_flag2 & (STK_FLG2_GS10_MASK | STK_FLG2_GS01_MASK)));
...
//          return;
        ...
        reading = stk3x1x_get_ps_reading(ps_data);
        if((ps_data->ps_distance_last != near_far_state)&&(ps_data->psi_set))
        {
            ps_data->ps_distance_last = near_far_state;
            input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE, near_far_state);
            input_event(ps_data->ps_input_dev,EV_SYN, SYN_TIME_SEC,ktime_to_timespec(timestamp).tv_sec);
...
static int stk3x1x_power_ctl(struct stk3x1x_data *data, bool on)
{
    ...
    if (!on && data->power_enabled) {
        ret = proximity_pinctrl_select(false,data);
        if (ret < 0)
            printk(KERN_INFO "[stk3x1x]stk3x1x_pinctrl_select fail\n");
        ...
        data->power_enabled = on;
        //set pin active
        ret = proximity_pinctrl_select(true,data);
        if (ret < 0)
            dev_err(&data->client->dev,"[stk3x1x]stk3x1x_pinctrl_select fail\n");
...
static int stk3x1x_set_input_devices(struct stk3x1x_data *ps_data)
{
    ...
    err = sysfs_create_link(ps_data->als_input_dev->dev.kobj.parent,&ps_data->als_input_dev->dev.kobj,ps_data->als_input_dev->name);
    if (err < 0) {
        printk(KERN_ERR "[ALS] %s Can't create soft link in touch driver\n", __func__);
    }
    ...
    err = sysfs_create_link(ps_data->ps_input_dev->dev.kobj.parent,&ps_data->ps_input_dev->dev.kobj,ps_data->ps_input_dev->name);
    if (err < 0) {
        printk(KERN_ERR "[ALS] %s Can't create soft link in touch driver\n", __func__);
    }
...
int proximity_pinctrl_select(bool on, struct stk3x1x_data *sensor)
{
    ...
    pins_state = on ? sensor->proximity_gpio_state_active : sensor->proximity_gpio_state_suspend;
    if (!IS_ERR_OR_NULL(pins_state)) {
        ret = pinctrl_select_state(sensor->proximity_pinctrl, pins_state);
        if (ret) {
            printk(KERN_INFO "can not set %s pins\n", on ? "proximity_active" : "proximity_suspend");
            return ret;
       }
    } else
...
int proximity_pinctrl_init(struct i2c_client *client,struct stk3x1x_data *sensor )
{
    int retval;
    /* Get pinctrl if target uses pinctrl */
    sensor->proximity_pinctrl = devm_pinctrl_get(&(client->dev));
    if (IS_ERR_OR_NULL(sensor->proximity_pinctrl)) {
        printk(KERN_INFO "Target does not use pinctrl\n");
        retval = PTR_ERR(sensor->proximity_pinctrl);
        sensor->proximity_pinctrl = NULL;
        return retval;
    }
    sensor->proximity_gpio_state_active = pinctrl_lookup_state(sensor->proximity_pinctrl, "proximity_active");
    if (IS_ERR_OR_NULL(sensor->proximity_gpio_state_active)) {
        printk(KERN_INFO "Can not get ts default pinstate\n");
        retval = PTR_ERR(sensor->proximity_gpio_state_active);
        sensor->proximity_pinctrl = NULL;
        return retval;
    }
    sensor->proximity_gpio_state_suspend = pinctrl_lookup_state(sensor->proximity_pinctrl, "proximity_suspend");
    if (IS_ERR_OR_NULL(sensor->proximity_gpio_state_suspend)) {
        printk(KERN_INFO "Can not get ts sleep pinstate\n");
        retval = PTR_ERR(sensor->proximity_gpio_state_suspend);
        sensor->proximity_pinctrl = NULL;
        return retval;
    }
    return 0;
...
/*@ add for ALS Calibration*/
    read_als_ratio(ps_data);
    if(ps_data->als_ratio==0)
        ps_data->als_ratio=165;
//        ps_data->als_ratio=100;
    ps_data->als_uncal_lux=0;
    err = proximity_pinctrl_init(client,ps_data);
    if (!err && ps_data->proximity_pinctrl) {
        err = proximity_pinctrl_select(true,ps_data);
        if (err < 0)
            printk(KERN_INFO "[stk3x1x]stk3x1x_pinctrl_select fail\n");
    }
...
stk_ps_tune_zero_init(ps_data);
    ...
    if(err < 0){
        dev_err(&client->dev,"%s: stk3x1x_set_input_devices err\n", __func__);
...
//#ifdef CONFIG_HAS_EARLYSUSPEND
    ...
    ps_data->als_cdev.min_delay=MIN_ALS_POLL_DELAY_NS/1000;
