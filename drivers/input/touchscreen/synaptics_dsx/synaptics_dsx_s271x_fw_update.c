...
//win add for proc
#include <asm/uaccess.h>//Win add for tocuh panel firmware version compare
#include <linux/proc_fs.h>
//win add for proc
//win add for panel select start
extern int touch_panel_select;
//win add for panel select end
//win add for touch upgrade firmware coexistence start
#define FW_IMAGE_NAME "TP_Firmware_innolux.img"
#define FW_IMAGE_NAME_TIANMA "TP_Firmware.img"
#define FW_IMAGE_NAME_CTC	"TP_Firmware_ctc.img"
#define FW_IMAGE_NAME_CTC_FAX	"TP_Firmware_ctc_fax.img"
//win add for touch upgrade firmware coexistence end

#define DO_STARTUP_FW_UPDATE

static char fih_touch[32]={0};
extern unsigned int get_device_config_id;
extern int synaptics_rmi4_f54_init(struct synaptics_rmi4_data *rmi4_data);
extern int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data);
#define STARTUP_FW_UPDATE_DELAY_MS 8000 /* ms */
...
static ssize_t fwu_sysfs_do_reflash_show(struct device *dev,
...
static struct device_attribute attrs[] = {
	__ATTR(doreflash, (S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH),
			/*synaptics_rmi4_show_error*/fwu_sysfs_do_reflash_show,
			fwu_sysfs_do_reflash_store),
	__ATTR(writeconfig, (S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH),
			synaptics_rmi4_show_error,
			fwu_sysfs_write_config_store),
	__ATTR(readconfig, (S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH),
			synaptics_rmi4_show_error,
			fwu_sysfs_read_config_store),
	__ATTR(configarea, (S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH),
			synaptics_rmi4_show_error,
			fwu_sysfs_config_area_store),
	__ATTR(imagename, (S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH),
			synaptics_rmi4_show_error,
			fwu_sysfs_image_name_store),
	__ATTR(imagesize, (S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH),
			synaptics_rmi4_show_error,
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	dev_info(rmi4_data->pdev->dev.parent,
			...
			fwu->config_data[2],
			fwu->config_data[3]);
	//Win add for force upgrade firmware version is 0x219045 start
	if ((device_config_id > 0x100000) || image_config_id > device_config_id) {
		pr_info("device_config_id=%x Catch it to force upgrade to early version for version control.",device_config_id);
	//Win add for force upgrade firmware version is 0x219045 end
...
exit:
	...
	pr_info("F@TOUCH Touch Register0x04 F01_RMI_DATA00 Device Status value= 0x%d\n",buf[0]);
	if(buf[0] == 0x00)
	{
		pr_info("F@TOUCH Touch OK\n");
	}
	if(buf[0] == 0x81)
	{
		buf_w[0] = 0x80;
		pr_info("F@TOUCH Touch Device Status:0x81\n");
...
{
	//win add for touch upgrade firmware coexistence start
	int retval=0,retval_reg= 0;
	u32 address_t,address_v,fwid,fwid2;
	...
	{//Tianma
		strncpy(fwu->image_name, FW_IMAGE_NAME_TIANMA, MAX_IMAGE_NAME_LEN);
	}
	if(touch_panel_select==3 ||touch_panel_select==4)
	{//CTC
		strncpy(fwu->image_name, FW_IMAGE_NAME_CTC, MAX_IMAGE_NAME_LEN);
	}
	if(touch_panel_select==5)
	{//CTC
		strncpy(fwu->image_name, FW_IMAGE_NAME_CTC_FAX, MAX_IMAGE_NAME_LEN);
	}

	snprintf(firmware_name,sizeof(firmware_name),"/system/etc/firmware/%s",fwu->image_name);
	pr_info("F@TOUCH firmware Path = %s\n",firmware_name);

	retval_reg = synaptics_rmi4_reg_read(rmi4_data,0x09,buf,sizeof(buf));
	pr_debug("F@TOUCH Touch register 0x09 retval_reg = %d\n",retval_reg);

	fwid = buf[0]<<24 | buf[1]<<16 | buf[2]<<8 | buf[3];
	pr_info("F@TOUCH Phone Touch firmware config ID = %.4X\n",fwid);

	RawdataFile = filp_open(firmware_name, O_RDONLY, (S_IRUGO | S_IWUSR) );
	if(IS_ERR(RawdataFile))
	{
		pr_err("F@TOUCH : %s open %s = %ld\n", __func__,firmware_name, PTR_ERR(RawdataFile));
		retval = -1;
		goto exit;
	}
	image_data = kzalloc(0xFFFF, GFP_KERNEL);
	if(image_data)
	{
		memset(image_data, 0, sizeof(image_data));
		RawdataFile->f_op->read(RawdataFile, image_data, 65536, &RawdataFile->f_pos);
		address_t=image_data[11]<<24 | image_data[10]<<16 | image_data[9]<<8 | image_data[8];
		pr_info("F@TOUCH Upgrade Touch firmware image_Size = 0x%.8X\n",address_t);

		address_v = address_t+0x100;
		fwid2 = image_data[address_v]<<24 | image_data[address_v+1]<<16 | image_data[address_v+2]<<8 | image_data[address_v+3];
		pr_info("F@TOUCH Upgrade Touch firmware config ID address = 0x%.8X config ID = %.4X\n",address_v,fwid2);
		if(firmware_flag ==1)
		{
			pr_info("F@TOUCH Touch Firmware Image can be back to early version\n");
		...
		else
		{
			if( fwid2 > fwid)
			{
				pr_info("F@TOUCH Touch Firmware Image can be upgraded.\n");
				retval = fwu_start_reflash();
			}
			//Win add for force upgrade firmware version is 0x219045 start
			else if(fwid > 0x100000)
			{
				pr_info("F@TOUCH Touch firmware abnormal,need to upgrade .");
				retval = fwu_start_reflash();
			}
			//Win add for force upgrade firmware version is 0x219045 end
			else if (fwid2 == fwid)
			{
				pr_info("F@TOUCH Touch Firmware Image version equal.\n");
				retval = fwu_start_reflash();
			}
			else
			{
				pr_notice("F@TOUCH Touch firmware image can't be upgraded.Version earlier.\n");
				retval = 0;
			}
		}
	}
	...
	{
		pr_info("F@TOUCH : %s alloc image_data failed.\n", __func__);
		goto err_handling;
	}
...
err_handling:
	if(image_data)
	{
		pr_info("F@TOUCH : %s kfree(image_data);\n", __func__);
		kfree(image_data);
	}
	if(RawdataFile)
	{
		pr_info("F@TOUCH : %s filp_close(RawdataFile);\n", __func__);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	{
		pr_info("F@TOUCH %s firmware upgrading.\n",__func__);
		return;
	}
	pr_info("F@TOUCH %s start.\n", __func__);
	if(get_device_config_id == 0x0 || rmi4_data->flash_prog_mode == 1)
	{
		pr_info("F@TOUCH Device config id = %.4X rmi4_data->flash_prog_mode = %d\n",get_device_config_id,rmi4_data->flash_prog_mode);
		retval_reg = synaptics_fw_updater(NULL);
		if (retval_reg < 0)
		{
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to do reflash\n",
		...
		sysfs_remove_link(rmi4_data->input_dev->dev.kobj.parent,rmi4_data->input_dev->name);
		input_free_device(rmi4_data->input_dev);
		input_unregister_device(rmi4_data->input_dev);
		rmi4_data->input_dev = NULL;
		synaptics_rmi4_set_input_dev(rmi4_data);
		synaptics_rmi4_f54_init(rmi4_data);
		retval_reg = sysfs_create_link(rmi4_data->input_dev->dev.kobj.parent,
	...
	if(get_device_config_id > 0x100000)
	{
		pr_info("F@TOUCH Device config id = %.4X \n",get_device_config_id);
		retval_reg = synaptics_fw_updater(NULL);
		if (retval_reg < 0)
		{
			dev_err(rmi4_data->pdev->dev.parent,
		...
		pr_info("F@TOUCH : %s Recover firmware version = %.4X\n", __func__,get_device_config_id);
		...
		if(touch_panel_select ==3 || touch_panel_select==4 || touch_panel_select==5)
		{	//CTC panel
...
//win add for proc
int touch_upgrade_read(char *buf)
{
	sprintf(buf, "%d\n",fw_upgrade_flag);
	return 0;
}
//FihtdcCodeAlanHZChang@Add inno check, begin
static int fih_touch_fwimver_inno_check(void)
{
	struct file *RawdataFile = NULL;
	char *image_data=NULL;
	u32 address_t=0,address_v=0,fwid2=0;
	char firmware_name[64];
	mm_segment_t fs;
	int value_temp=0;
	int ret = 0;
	...
	if(touch_panel_select==1)
	{
		...
		snprintf(firmware_name,sizeof(firmware_name),"/system/etc/firmware/%s",fwu->image_name);
		pr_debug("F@TOUCH firmware Path = %s",firmware_name);
		RawdataFile = filp_open(firmware_name, O_RDONLY, (S_IRUGO | S_IWUSR) );
		if(IS_ERR(RawdataFile))
		{
			pr_err("F@TOUCH : %s open %s failed = %ld\n", __func__,firmware_name, PTR_ERR(RawdataFile));
			ret = -1;
			goto exit;
		}
		value_temp=RawdataFile->f_op->llseek(RawdataFile,0,SEEK_END);
		pr_debug("F@TOUCH firmware image_Size = %d",value_temp);
		image_data = kzalloc(value_temp, GFP_KERNEL);
		RawdataFile->f_op->llseek(RawdataFile,0,SEEK_SET);
		if(image_data)
		{
			memset(image_data, 0, sizeof(image_data));
			RawdataFile->f_op->read(RawdataFile, image_data, value_temp, &RawdataFile->f_pos);
			address_t=image_data[11]<<24 | image_data[10]<<16 | image_data[9]<<8 | image_data[8];
			pr_debug("F@TOUCH Upgrade Touch firmware image_Size = 0x%.8X",address_t);

			address_v = address_t+0x100;
			fwid2 = image_data[address_v]<<24 | image_data[address_v+1]<<16 | image_data[address_v+2]<<8 | image_data[address_v+3];
			pr_debug("F@TOUCH Upgrade Touch firmware config ID address = 0x%.8X config ID = %.4X",address_v,fwid2);
		}
		if(RawdataFile != NULL)
		{
			pr_debug("F@TOUCH : %s filp_close(RawdataFile);\n", __func__);
			filp_close(RawdataFile,NULL);
		}
		kzfree(image_data);
exit:
		set_fs(fs);
		//FW 100x can not upgrade to FW 200x, need to reture 0;
		if((get_device_config_id>0x10000) && (get_device_config_id<0x15000) && (fwid2 > 0x20000)) //Touch FW V1.00x
		{
			ret = 1; //Do not update
		}
	}
...
//FihtdcCodeAlanHZChang@Add inno check, end
int touch_upgrade_write(char *buf)
{
	ssize_t retval = 0;
...
//FihtdcCodeAlanHZChang@Add inno check, begin
	if(fih_touch_fwimver_inno_check() == 1)
	{
	    pr_err("F@TOUCH %s, DO NO UPGRADE for Inno Older version\n", __func__);
	    return retval_reg;
	}
//FihtdcCodeAlanHZChang@Add inno check, end

	input_char=kzalloc(strlen(buf),GFP_KERNEL);

	fw_upgrade_flag = -1;
	if (sscanf(buf, "%s", input_char) ==1)
		{
			input = NORMAL;
			if(memcmp(input_char,"1",1)==0)
			{
				input = NORMAL;
			}
			else if(memcmp(input_char,"2",1)==0)
			{
				input = FORCE;
			}
			else if(memcmp(input_char,"8",1)==0)
			{
				input = LOCKDOWN;
			}
		}
	...
	{
		pr_info("F@TOUCH input != 1");
		retval = -EINVAL;
		goto exit;
	}
#endif
	pr_info("F@TOUCH input_char  %s",input_char);
	pr_info("F@TOUCH %s input = %d\n", __func__, input);
	if (input & LOCKDOWN) {
	...
	if(get_device_config_id == 0x0)
	{
		pr_info("F@TOUCH reinit input device for damage fw reflash done.\n");
		sysfs_remove_link(rmi4_data->input_dev->dev.kobj.parent,rmi4_data->input_dev->name);
		input_free_device(rmi4_data->input_dev);
		input_unregister_device(rmi4_data->input_dev);
		rmi4_data->input_dev = NULL;
	...
	get_device_config_id = get_config_id[0]<<24 | get_config_id[1]<<16 | get_config_id[2]<<8 | get_config_id[3];
	pr_info("F@TOUCH : %s firmware version = %.4X\n", __func__,get_device_config_id);
	memset(fih_touch, 0, sizeof(fih_touch));
	if(touch_panel_select ==1)
	{	//innolux panel
		snprintf(fih_touch,PAGE_SIZE, "Synaptics-IV%.4X\n",get_device_config_id);
...
int touch_fwback_read(char *buf)
{
	sprintf(buf, "%d\n",firmware_flag);
	return 0;
}

int touch_fwback_write(void)
{
	firmware_flag=1;
	return 0;
}

int touch_fwimver_read(char *buf)
{
	struct file *RawdataFile = NULL;
	char *image_data=NULL;
	u32 address_t=0,address_v=0,fwid2=0;
	...
	if(fwu == NULL)
	{
		pr_info("F@TOUCH fwu is not exist");
		return 0;
	}
	if(touch_panel_select==1)
	{//innolux
...
exit:
	set_fs(fs);
	/*FihtdcCode@AlanHZChang, skip FW version while Inno fw is older, begin*/
	if(touch_panel_select==1)
	{
	    //FW 100x can not upgrade to FW 200x, need to reture 0;
        if((get_device_config_id>0x10000) && (get_device_config_id<0x15000) && (fwid2 > 0x20000)) //Touch FW V1.00x
        {
            pr_err("F@TOUCH Upgrade Touch get_device_config_id = %d config ID = %d",get_device_config_id,fwid2);
            fwid2 = 0;
        }
    }
    /*FihtdcCode@AlanHZChang, skip FW version while Inno fw is older, begin*/
...
/*
static const struct file_operations touch_upgrade_proc_fops = {
	.owner = THIS_MODULE,
	.read = touch_upgrade_read,
	.write = touch_upgrade_write,
};

static const struct file_operations touch_fwback_proc_fops = {
	.owner = THIS_MODULE,
	.read = touch_fwback_read,
	.write = touch_fwback_write,
};

static const struct file_operations touch_fwimver_proc_fops = {
	.owner = THIS_MODULE,
	.read = touch_fwimver_read,
};
*/
//win add for proc
static ssize_t fwu_sysfs_do_reflash_show(struct device *dev, struct device_attribute *attr, char *buf)
{
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &input) != 1)
	{
		pr_notice("F@TOUCH input != 1");
		retval = -EINVAL;
		goto exit;
	}
#endif
	pr_notice("F@TOUCH input_char  %s",input_char);
	pr_notice("F@TOUCH %s input = %d\n", __func__, input);
	if (input & LOCKDOWN) {
		fwu->do_lockdown = true;
		input &= ~LOCKDOWN;
	}
