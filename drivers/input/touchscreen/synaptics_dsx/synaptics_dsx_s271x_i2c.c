...
int ex_ldo_en_gpio;
EXPORT_SYMBOL(ex_ldo_en_gpio);	//2014.02.12 win add for turn off external ldo
...
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_dsx_board_data *pdata)
{
	...
	pr_debug("F@TOUCH %s reg_en = %d\n", __func__, pdata->reg_en);
	pdata->i2c_pull_up = of_property_read_bool(np, "synaptics,i2c-pull-up");
	pr_debug("F@TOUCH %s i2c_pull_up = %d\n", __func__, pdata->i2c_pull_up);
	/* reset, irq gpio info */

	pdata->irq_gpio = of_get_named_gpio_flags(np, "synaptics,irq-gpio", 0, &pdata->irq_flags);
	pr_debug("F@TOUCH %s irq_gpio = %d\n", __func__, pdata->irq_gpio);
	
	//win add for enable external ldo temporary before regulator L15 enable to 3.1V start
	ex_ldo_en_gpio = of_get_named_gpio_flags(np, "synaptics,ex-ldo-en-gpio", 0, NULL);
	pr_debug("F@TOUCH %s ex_ldo_en_gpio = %d\n", __func__, ex_ldo_en_gpio);
	if(gpio_request(ex_ldo_en_gpio, "ex_ldo_en")){
		pr_err("F@TOUCH %s ex_ldo_en_gpio reguest fail\n", __func__);
	}
	//win add for enable external ldo temporary before regulator L15 enable to 3.1V end
	...
	pr_debug("F@TOUCH %s power_gpio = %d\n", __func__, pdata->power_gpio);
	...
	if (prop && touch_panel_select ==4) {
		pr_info("F@TOUCH %s find button-map\n",__func__);
		...
		const struct i2c_device_id *dev_id)
{
	...
	//win add for enable external ldo temporary before regulator L15 enable to 3.1V start
	gpio_direction_output(ex_ldo_en_gpio, 1);
	gpio_set_value(ex_ldo_en_gpio,1);
	if(gpio_get_value(ex_ldo_en_gpio) == 0)
		pr_err("F@TOUCH %s ex_ldo_en_gpio didn't be enabled Value = %d .\n", __func__,gpio_get_value(ex_ldo_en_gpio));
	//win add for enable external ldo temporary before regulator L15 enable to 3.1V end
	pr_debug("F@TOUCH %s end.\n", __func__);
	return 0;
}
