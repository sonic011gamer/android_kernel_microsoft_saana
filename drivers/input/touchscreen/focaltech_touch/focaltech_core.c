...
extern void touch_tp_alt_st_enable(int);
extern int touch_tp_alt_st_count(void);
extern void touch_tp_alt_rst(int);
    ...
    for (i = 0; i < event->touch_point; i++)
    {
	  if((event->au16_x[i]!=0) && (event->au16_y[i]!=0)){ //20170421 JanChen
        ...
        if (event->au8_touch_event[i] == FTS_TOUCH_DOWN || event->au8_touch_event[i] == FTS_TOUCH_CONTACT)
        {
          ...
          input_report_abs(data->input_dev, ABS_MT_TRACKING_ID, event->au8_finger_id[i]); //20170502 JanChen
#if FTS_REPORT_PRESSURE_EN
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    int err,no_tp=0;
    printk(KERN_ERR "[FTS] Entet %s", __func__);
    FTS_FUNC_ENTER();
    /* 1. Get Platform data */
    ...
    fts_reset_proc(200);
    no_tp=fts_wait_tp_to_valid(client);
    if (no_tp < 0)
    {
        FTS_ERROR("no_tp > 1, TP does not exist");
        goto free_gpio;
    }
//
...
/*
	...
	// read TP FW end
    printk(KERN_ERR "[FTS][Error]fts_ts_probe success \n");
    FTS_FUNC_EXIT();
    return 0;
...
*  Return:
*****************************************************************************/
	void fih_fts_tp_lcm_suspend(void)
{
    FTS_ERROR("TP call %s from lcm",__func__);
    fts_ts_suspend(&fts_input_dev->dev);
    return ;
}
EXPORT_SYMBOL(fih_fts_tp_lcm_suspend);
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if (data->suspended)
    {
        FTS_ERROR("Already in suspend state");
        FTS_FUNC_EXIT();
        return -1;
    }
#if FTS_GESTURE_EN
...
*  Return:
*****************************************************************************/
void fih_fts_tp_lcm_resume(void)
{
    FTS_ERROR("TP call %s from lcm",__func__);
    fts_ts_resume(&fts_input_dev->dev);
    return ;
}
EXPORT_SYMBOL(fih_fts_tp_lcm_resume);
