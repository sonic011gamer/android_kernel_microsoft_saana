...
#define FTXXXX_INI_FILENAME_CONFIG "TP_Firmware"
...
void touch_e1m_fwimver_read(char *fw_imver)
{
	u8 fwimver = 0;

	mutex_lock(&fts_input_dev->mutex);

	fwimver = fts_ctpm_get_app_bin_file_ver( FTXXXX_INI_FILENAME_CONFIG);

    	if (!fwimver)
	{
		fwimver = 00;
	}
    	//retuen FB is null file
	snprintf(fw_imver, 128, "%02X\n", fwimver);
	pr_err("%s",fw_imver);
	mutex_unlock(&fts_input_dev->mutex);
}

void touch_tpfwver_read(char *fih_touch)
{
    u8 fwver = 0;
    u8 fwimver = 0;
	...
	snprintf(fih_touch, 128, "FocalTech-V%02X_%02X\n", fwver, fwimver);
	pr_err("%s",fih_touch);
...
int gForceUpgrade = 0;
void touch_upgrade(int input)
{
	char fih_touch_temp[32]={0};
	mutex_lock(&fts_input_dev->mutex);
	...
	fw_upgrade_flag = fts_ctpm_fw_upgrade_with_app_bin_file(fts_i2c_client, FTXXXX_INI_FILENAME_CONFIG);

	enable_irq(fts_i2c_client->irq);
	mutex_unlock(&fts_input_dev->mutex);
	touch_tpfwver_read(fih_touch_temp);
}
