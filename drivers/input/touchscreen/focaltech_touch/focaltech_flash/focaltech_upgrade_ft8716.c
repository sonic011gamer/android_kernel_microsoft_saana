...
extern int gForceUpgrade;	//SW4-HL-TouchPanel-ImplementFirmwareUpgradeAndGetFirmwareVersion-01+_20151124
/*****************************************************************************
...
* Return: fw version
***********************************************************************/
int fts_ctpm_get_app_bin_file_ver(char *firmware_name)
{
    u8 *pbt_buf = NULL;
...
* Return: fw version
***********************************************************************/
int fts_ctpm_get_app_i_file_ver(void)
{
...
* Return: fail < 0
***********************************************************************/
int fts_ctpm_fw_upgrade_with_app_i_file(struct i2c_client *client)
{
    int i_ret = 0;
    u32 fw_len;
...
* Return: success =0
***********************************************************************/
int fts_ctpm_fw_upgrade_with_app_bin_file(struct i2c_client *client, char *firmware_name)
{
    u8 *pbt_buf = NULL;
    int retry_cnt, i_ret=0;
    bool ecc_ok = false;
    int fwsize = fts_GetFirmwareSize(firmware_name);
    unsigned char fw_ver_of_firmware_file = 0;
    unsigned char fw_ver_of_ic = 0;
...
///////////////////////////////////////////////////////////////
	fw_ver_of_firmware_file = pbt_buf[APP_FILE_VER_MAPPING];
	
	//Get IC's firmware version
	if (fts_i2c_read_reg(client, FTS_REG_FW_VER, &fw_ver_of_ic) < 0)
	{
	...
	pr_err("\n\n[HL]%s: fw_ver_of_firmware_file = %d\n\n", __func__, fw_ver_of_firmware_file);
	pr_err("\n\n[HL]%s: fw_ver_of_ic = %d\n\n", __func__, fw_ver_of_ic);
	if (!gForceUpgrade)
	{
		if (fw_ver_of_firmware_file > fw_ver_of_ic)
		{
			pr_err("fw_ver_of_firmware_file > fw_ver_of_ic, START to upgrade tp firmware!\n");

			goto upgrade_start;
		}
		else
		{
			pr_err("fw_ver_of_firmware_file <= fw_ver_of_ic, IGNORE to upgrade tp firmware!\n");
			goto upgrade_ignore;
		}
	}
	else
...
///////////////////////////////////////////////////////////////
    ...
    else
    {
        ...
        goto ERROR_BIN;
        	...
        	pr_err("[FTS] %s: upgrade_starting end\n", __func__);
    kfree(pbt_buf);
    return i_ret;
...
upgrade_start:
	...
	for (retry_cnt = 0; retry_cnt < 3; retry_cnt++) {
        	pr_err("[FTS] %s: upgrade_starting %d......\n", __func__,retry_cnt);
	    i_ret = fts_ctpm_fw_upgrade_use_buf(client, pbt_buf, fwsize);
	    if (i_ret != 0){
	        dev_err(&client->dev, "%s() - ERROR:[FTS] upgrade failed.., retry cnt %d \n",
					__func__,retry_cnt);
	        msleep(20);
		    }
		else
	...
	{
	dev_err(&client->dev, "%s() - [FTS] upgrade succeed!\n",
					__func__);
	}
