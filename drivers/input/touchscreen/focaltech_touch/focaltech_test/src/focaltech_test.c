...
#define FTS_test_FILE_PATH "/data/misc/touch/"

//#define FTXXXX_INI_FILENAME_CONFIG "TP_Firmware_RBH"
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    FTS_TEST_ERROR("ini_file_name = %s", cfgname);
...
static ssize_t fts_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    fwname[count-1] = '\0';
    FTS_TEST_ERROR("fwname:%s.", fwname);
printk(KERN_ERR "[FTS][TEST][Error]fwname:%s \n", fwname);
...
void touch_selftest(void)
{
    ...
    memset(fwname, 0, sizeof(fwname));
    //FTXXXX_INI_CONFIG_NAME is ini file name
    sprintf(fwname, "%s", FTXXXX_INI_CONFIG_NAME);
