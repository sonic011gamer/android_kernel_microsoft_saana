...
#define ORIENT_INTERRUPT            ABS_MISC
...
struct bosch_sensor_specific bma2x2_platform;
/*!
...
extern int fih_gsensor_proc_init(void);
...
static void bma2x2_work_func(struct work_struct *work)
{
	...
	err = bma2x2_read_accel_xyz(bma2x2->bma2x2_client, bma2x2->sensor_type, &acc);
	if(err != 0)
		...
		const char *buf, size_t count)
{
    ...
    printk(KERN_INFO "[G-sensor]%s enable = %ld\n", __func__, data);
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);
	signed char place = bma2x2_platform.place; 
...
static DEVICE_ATTR(enable_timer, S_IWUSR|S_IWGRP,
		bma2x2_enable_timer_show, bma2x2_enable_timer_store);
static DEVICE_ATTR(debug_level, S_IWUSR|S_IWGRP,
...
static int bma2x2_pinctrl_init(struct i2c_client *client)
{
	int retval;
	...
	data->gpio_state_active
		= pinctrl_lookup_state(data->ts_pinctrl, "bma2x2_active");
	...
	data->gpio_state_suspend
		= pinctrl_lookup_state(data->ts_pinctrl, "bma2x2_suspend");
...
static int bma2x2_pinctrl_select(struct bma2x2_data *data,
						bool on)
{
	...
	if (!IS_ERR_OR_NULL(pins_state)) {
		...
		if (ret) {
			printk(KERN_INFO "can not set %s pins\n",
				on ? "bma2x2_active" : "bma2x2_suspend");
			return ret;
		}
	} else
...
void bma2x2_config_gpio(struct i2c_client *client)
{
	int err;
  struct bma2x2_data *data = i2c_get_clientdata(client);
  ...
  u32 temp_val;
	...
	err = bma2x2_pinctrl_init(client);
	if (!err && data->ts_pinctrl) {
		err = bma2x2_pinctrl_select(data, true);
		if (err < 0)
			PERR("[Sensor]bma2x2_pinctrl_select fail\n");
	}

	bma2x2_platform.name = "bma2x2";
	//bma2x2_platform.place = 0;
	...
	if (temp_val > 7 || temp_val < 0) {
		...
		bma2x2_platform.place = temp_val;
	}
	PERR( "bma2x2_platform.place = %d\n",bma2x2_platform.place);
	bma2x2_platform.irq = of_get_named_gpio_flags(np, "bosch,irq1-gpio", 0, NULL);
	err = gpio_request(bma2x2_platform.irq, "bma2x2_3V");
	if (err) {
		PERR("%s: Failed to get gpio %d (code: %d)",
				__func__, bma2x2_platform.irq, err);
	}
	err = gpio_direction_input(bma2x2_platform.irq);
	if (err) {
		PERR("%s: Failed to set gpio %d direction",
				__func__, bma2x2_platform.irq);
	}
	bma2x2_platform.mux_vcc = regulator_get(&client->dev, "mux_vcc");

	if(IS_ERR(bma2x2_platform.mux_vcc))
		printk(KERN_INFO "[G-sensor]regulator_get mux_vcc fail\n");	

	bma2x2_platform.vcc_i2c = regulator_get(&client->dev, "vcc_i2c");

	if(IS_ERR(bma2x2_platform.vcc_i2c))
		printk(KERN_INFO "[G-sensor]regulator_get vcc_i2c fail\n");

	ret = regulator_enable(bma2x2_platform.mux_vcc);
	if (ret)
		printk(KERN_INFO "[G-sensor]regulator_enable mux_vcc fail\n");

	ret = regulator_enable(bma2x2_platform.vcc_i2c);
	if (ret)
		printk(KERN_INFO "[G-sensor]regulator_enable vcc_i2c fail\n");
}

void bma2x2_gpio_free(void)
{
	int ret;
	printk(KERN_INFO "[G-sensor]bma2x2_gpio_free enter\n");
	gpio_set_value(bma2x2_platform.irq, 0);
	gpio_free(bma2x2_platform.irq);
	ret = regulator_disable(bma2x2_platform.vcc_i2c);

	if (ret)
		printk(KERN_INFO "[G-sensor]regulator_disable vcc_i2c fail\n");
	regulator_put(bma2x2_platform.vcc_i2c);

	ret = regulator_disable(bma2x2_platform.mux_vcc);
	if (ret)
		printk(KERN_INFO "[G-sensor]regulator_disable mux_vcc fail\n");
	regulator_put(bma2x2_platform.mux_vcc);
}
		...
		const struct i2c_device_id *id)
{
	...
	bma2x2_config_gpio(client);
	pdata = &bma2x2_platform;
	...
	err = sysfs_create_link(dev->dev.kobj.parent,
				...
				dev->name);
	if (err < 0) {
		printk(KERN_INFO "[G-sensor] %s Can't create soft link in g-sensor driver\n", __func__);
	}
...
err_register_input_device_interrupt:
	sysfs_remove_link(dev->dev.kobj.parent, dev->name);
...
kfree_exit:
	bma2x2_gpio_free();
	if (data->ts_pinctrl) {
		err = bma2x2_pinctrl_select(data, false);
		if (err < 0)
			PERR("Cannot get idle pinctrl state\n");
...
static int bma2x2_remove(struct i2c_client *client)
{
	...
	if (NULL == data)
		...
		printk(KERN_INFO "[G-sensor]bst_unregister_device\n");
		/*bst_free_device(data->bst_acc);
		printk(KERN_INFO "[G-sensor]bst_free_device\n");*/
		bma2x2_gpio_free();
...
static int bma2x2_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (data->ts_pinctrl)
		bma2x2_pinctrl_select(data, false);
	if (data->is_timer_running) {
		hrtimer_cancel(&data->timer);
		data->base_time = 0;
		data->timestamp = 0;
...
static int bma2x2_resume(struct i2c_client *client)
{
	...
	mutex_lock(&data->enable_mutex);
	if (data->ts_pinctrl)
		bma2x2_pinctrl_select(data, true);
	if (atomic_read(&data->enable) == 1) {
		bma2x2_set_mode(data->bma2x2_client,
			BMA2X2_MODE_NORMAL, BMA_ENABLED_INPUT);
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
