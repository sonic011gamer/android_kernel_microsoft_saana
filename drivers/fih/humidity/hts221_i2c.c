...
struct bosch_humidity_specific hts221_platform;
...
void hts221_config_gpio(struct i2c_client *client)
{
	int ret;

	hts221_platform.mux_vcc = regulator_get(&client->dev, "mux_vcc");
	if(IS_ERR(hts221_platform.mux_vcc))
		printk(KERN_INFO "[Humidity]regulator_get mux_vcc fail\n");

	hts221_platform.vcc_i2c = regulator_get(&client->dev, "vcc_i2c");
	if(IS_ERR(hts221_platform.vcc_i2c))
		printk(KERN_INFO "[Humidity]regulator_get vcc_i2c fail\n");
	ret = regulator_enable(hts221_platform.mux_vcc);
	if (ret)
		printk(KERN_INFO "[Humidity]regulator_enable mux_vcc fail\n");

	ret = regulator_enable(hts221_platform.vcc_i2c);
	if (ret)
		printk(KERN_INFO "[Humidity]regulator_enable vcc_i2c fail\n");
}

void hts221_gpio_free(void)
{
	int ret;
	printk(KERN_INFO "[Humidity]hts221_gpio_free enter\n");

	ret = regulator_disable(hts221_platform.vcc_i2c);

	if (ret)
		printk(KERN_INFO "[Humidity]regulator_disable vcc_i2c fail\n");
	regulator_put(hts221_platform.vcc_i2c);

	ret = regulator_disable(hts221_platform.mux_vcc);
	if (ret)
		printk(KERN_INFO "[Humidity]regulator_disable mux_vcc fail\n");
	regulator_put(hts221_platform.mux_vcc);
}
