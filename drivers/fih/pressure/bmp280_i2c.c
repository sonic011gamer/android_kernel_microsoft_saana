...
struct bosch_pressure_specific bmp280_platform;
...
void bmp280_config_gpio(struct i2c_client *client)
{
	int ret;

	bmp280_platform.mux_vcc = regulator_get(&client->dev, "mux_vcc");
	if(IS_ERR(bmp280_platform.mux_vcc))
		printk(KERN_INFO "[Pressure]regulator_get mux_vcc fail\n");

	bmp280_platform.vcc_i2c = regulator_get(&client->dev, "vcc_i2c");
	if(IS_ERR(bmp280_platform.vcc_i2c))
		printk(KERN_INFO "[Pressure]regulator_get vcc_i2c fail\n");
	ret = regulator_enable(bmp280_platform.mux_vcc);
	if (ret)
		printk(KERN_INFO "[Pressure]regulator_enable mux_vcc fail\n");

	ret = regulator_enable(bmp280_platform.vcc_i2c);
	if (ret)
		printk(KERN_INFO "[Pressure]regulator_enable vcc_i2c fail\n");
}

void bmp280_gpio_free(void)
{
	int ret;
	printk(KERN_INFO "[Pressure]bmp280_gpio_free enter\n");

	ret = regulator_disable(bmp280_platform.vcc_i2c);

	if (ret)
		printk(KERN_INFO "[Pressure]regulator_disable vcc_i2c fail\n");
	regulator_put(bmp280_platform.vcc_i2c);

	ret = regulator_disable(bmp280_platform.mux_vcc);
	if (ret)
		printk(KERN_INFO "[Pressure]regulator_disable mux_vcc fail\n");
	regulator_put(bmp280_platform.mux_vcc);
}
				...
				const struct i2c_device_id *id)
{
    ...
    printk(KERN_INFO "[Pressure]%s Enter\n", __func__);
...
static struct of_device_id bmp280_match_table[] = {
	{ .compatible = BMP_NAME,},
	{ },
};
