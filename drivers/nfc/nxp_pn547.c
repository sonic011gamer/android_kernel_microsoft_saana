...
unsigned char nci_reset_nci_resp[] = {0x40, 0x00, 0x03, 0x00, 0x10, 0x01};//Send U-RSET frame
...
static int nxp_pn547_pinctrl_init(struct pn547_dev *pn547_dev)
{
  struct i2c_client *client = pn547_dev->client;
  
	pn547_dev->pinctrl = devm_pinctrl_get(&client->dev);
	if (IS_ERR_OR_NULL(pn547_dev->pinctrl)) {
		dev_err(&client->dev, "Failed to get pinctrl\n");
		return PTR_ERR(pn547_dev->pinctrl);
	}
	
	pn547_dev->pins_active =
		pinctrl_lookup_state(pn547_dev->pinctrl, "nfc_active");
	if (IS_ERR_OR_NULL(pn547_dev->pins_active)) {
		dev_err(&client->dev, "Failed to look up default state\n");
		return PTR_ERR(pn547_dev->pins_active);
	}

	pn547_dev->pins_sleep =
		pinctrl_lookup_state(pn547_dev->pinctrl, "nfc_suspend");
	if (IS_ERR_OR_NULL(pn547_dev->pins_sleep)) {
		dev_err(&client->dev, "Failed to look up suspend state\n");
		return PTR_ERR(pn547_dev->pins_sleep);
	}
	return 0;
}

int check_chip_alive(struct pn547_dev *pn547_dev)
{
	int i = 0 ,irq_status = 0 ,ret = 0;
	char tmp[10];
	...
	ret = i2c_master_send(pn547_dev->client, nci_reset_nci, 4);
	if(ret < 0)
	{
		pr_err("%s : i2c_master_send returned %d\n", __func__, ret);
		return ret;
	...
	pr_info("%s : irq_gpio = %d\n", __func__, irq_status);
	//read I2C response
	ret = i2c_master_recv(pn547_dev->client, tmp, 6);
	if(ret < 0)
	{
		pr_err("%s : i2c_master_recv returned %d\n", __func__, ret);
		return ret;
		...
		const struct i2c_device_id *id)
{
	...
	if (!platform_data)
		return -EINVAL;
	pr_info("nfc-pn547 probe: %s, inside nfc-pn547 flags = %x\n",__func__, client->flags);
	...
	/* initialize pinctrl */
	ret = nxp_pn547_pinctrl_init(pn547_dev);
	if (ret) {
		dev_err(&client->dev, "Can't initialize pinctrl\n");
			goto err_ven;
	}

	ret = pinctrl_select_state(pn547_dev->pinctrl, pn547_dev->pins_active);
	if (ret) {
		dev_err(&client->dev,
	...
	pr_info("%s irq_gpio=%d, ven_gpio=%d, firm_gpio=%d\n", __func__,
	...
	ret = check_chip_alive(pn547_dev);
	if (ret < 0)
	{
		pr_err("F@NFC %s fail.",__func__);
		goto err_request_irq_failed;
	}
	else
	{
