...
static void fih_msm_sensor_restart_stream(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->sensor_state == MSM_SENSOR_POWER_UP)
	{
		...
		//mi++ Front camera data type is BYTE
		if(strncmp(sensor_name, "s5k5e8yx13_e1m", strlen("s5k5e8yx13_e1m"))==0){
	...
	void __user *argp)
{
	...
	case CFG_SLAVE_READ_I2C: {
		...
		read_addr_type = read_config.addr_type;
                //pr_err("[MIMI]%s:CFG_SLAVE_READ_I2C:%d \n", __func__, __LINE__);
                //pr_err("[MIMI]%s:slave_addr=0x%x reg_addr=0x%x, data_type=%d \n",__func__, read_config.slave_addr,
...
//mi++E1M5949 avoid kerenl panic when open camera
...
int msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	case CFG_SLAVE_READ_I2C: {
		...
		read_slave_addr = read_config.slave_addr;
		read_addr_type = read_config.addr_type;
                //pr_err("[MIMI]%s:111CFG_SLAVE_READ_I2C:%d \n", __func__, __LINE__);
                //pr_err("[MIMI]%s:111slave_addr=0x%x reg_addr=0x%x, data_type=%d \n",__func__, read_config.slave_addr,
			//read_config.reg_addr, read_config.data_type);
