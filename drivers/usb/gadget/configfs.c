...
/* FIH - Michael - Porting fih scsi command START*/
...
static ssize_t gadget_dev_desc_idProduct_store(struct gadget_info *gi,
		const char *page, size_t len)
{
	u16 val;
	int ret;
	...
	if (ret)
		return ret;
	gi->cdev.desc.idProduct = cpu_to_le16p(&val);
...
/* FIH - Michael - Porting fih scsi command END*/
